...
enum ParameterType { Text, Integer, Choice, Boolean, Path, Secret }
...
#[derive(Debug, Clone)]
struct LoadedRunbook { key: String, source: PathBuf, runbook: RunbookFile }

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct TrustRecord { path: String, digest: String, signed_at: DateTime<Utc>, signature: String }

#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
struct TrustedDirectory { path: String, digest: String, signed_at: DateTime<Utc>, valid: bool, error: Option<String> }
...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
struct DirectoryInspection { path: String, digest: String, files: Vec<String>, runbooks: Vec<RunbookSummary>, warnings: Vec<String> }
...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
struct PreparedRun { runbook_id: String, name: String, risk: String, rollback: String, steps: Vec<PreparedStep> }

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct RunResult {
    id: String, runbook_id: String, name: String, started_at: DateTime<Utc>, duration_ms: u128,
    status: String, exit_code: Option<i32>, output: String, rollback: String,
}

fn app_data_dir() -> AppResult<PathBuf> {
    let base = dirs::data_local_dir().ok_or_else(|| AppError::Message("This platform does not provide a local app-data directory.".into()))?;
...
fn signing_key() -> AppResult<Vec<u8>> {
    ...
    if path.exists() { return Ok(fs::read(path)?); }
...
fn sign(path: &str, digest: &str) -> AppResult<String> {
    let mut mac = HmacSha256::new_from_slice(&signing_key()?).map_err(|_| AppError::Message("Could not initialize local signature.".into()))?;
    mac.update(path.as_bytes()); mac.update(&[0]); mac.update(digest.as_bytes());
    Ok(hex::encode(mac.finalize().into_bytes()))
}

fn verify_record(record: &TrustRecord) -> AppResult<bool> { Ok(sign(&record.path, &record.digest)? == record.signature) }

fn trust_records() -> AppResult<Vec<TrustRecord>> { read_json(&app_data_dir()?.join("trusted-directories.json")) }
fn save_trust_records(records: &[TrustRecord]) -> AppResult<()> { write_json(&app_data_dir()?.join("trusted-directories.json"), &records) }

fn assert_owned(path: &Path) -> AppResult<Vec<String>> {
    let metadata = fs::symlink_metadata(path)?;
    if !metadata.is_dir() { return Err(AppError::Message("Choose a folder, not a file.".into())); }
    if metadata.file_type().is_symlink() { return Err(AppError::Message("Symlinked runbook folders are not accepted.".into())); }
    let warnings = Vec::new();
    #[cfg(unix)]
    {
        use std::os::unix::fs::MetadataExt;
        if metadata.uid() != unsafe { libc::geteuid() } { return Err(AppError::Message("The selected folder is not owned by your current user.".into())); }
...
fn yaml_files(path: &Path) -> AppResult<Vec<PathBuf>> {
    ...
    for entry in WalkDir::new(path).max_depth(3).follow_links(false) {
        let entry = entry.map_err(|error| AppError::Message(format!("Could not inspect the folder: {error}")))?;
        if entry.file_type().is_symlink() { return Err(AppError::Message(format!("Symlinks are not accepted: {}", entry.path().display()))); }
        if entry.file_type().is_file() && matches!(entry.path().extension().and_then(|v| v.to_str()), Some("yaml" | "yml")) {
            if entry.metadata().map_err(|e| AppError::Message(e.to_string()))?.len() > MAX_FILE_BYTES { return Err(AppError::Message(format!("{} is larger than 64 KB.", entry.path().display()))); }
            files.push(entry.path().to_path_buf());
        }
    }
    files.sort();
    if files.is_empty() { return Err(AppError::Message("No .yaml or .yml runbooks were found within three folder levels.".into())); }
    if files.len() > 100 { return Err(AppError::Message("A trusted folder may contain at most 100 YAML files.".into())); }
    Ok(files)
}
...
fn directory_digest(root: &Path, files: &[PathBuf]) -> AppResult<String> {
    ...
    for file in files {
        let relative = file.strip_prefix(root).map_err(|_| AppError::Message("A runbook escaped its selected folder.".into()))?;
        hasher.update(relative.to_string_lossy().as_bytes()); hasher.update([0]); hasher.update(fs::read(file)?); hasher.update([0]);
    }
    Ok(hex::encode(hasher.finalize()))
}

fn validate_runbook(runbook: &RunbookFile, source: &Path) -> AppResult<()> {
    if runbook.version != 1 { return Err(AppError::Message(format!("{} uses unsupported version {}.", source.display(), runbook.version))); }
    let id_re = Regex::new(r"^[a-z0-9][a-z0-9-]{1,62}$").unwrap();
    let name_re = Regex::new(r"^[A-Za-z][A-Za-z0-9_]{0,31}$").unwrap();
    if !id_re.is_match(&runbook.id) { return Err(AppError::Message(format!("{} has an invalid id.", source.display()))); }
    if runbook.name.trim().is_empty() || runbook.description.trim().is_empty() || runbook.rollback.trim().is_empty() { return Err(AppError::Message(format!("{} needs a name, description, and rollback note.", source.display()))); }
    if !matches!(runbook.risk.as_str(), "low" | "medium" | "high") { return Err(AppError::Message(format!("{} risk must be low, medium, or high.", source.display()))); }
    if runbook.steps.is_empty() || runbook.steps.len() > 20 { return Err(AppError::Message(format!("{} needs 1–20 steps.", source.display()))); }
    let mut names = std::collections::HashSet::new();
    for parameter in &runbook.parameters {
        if !name_re.is_match(&parameter.name) || !names.insert(parameter.name.clone()) { return Err(AppError::Message(format!("{} has an invalid or duplicate parameter name.", source.display()))); }
        if parameter.kind == ParameterType::Choice && parameter.choices.as_ref().map_or(true, |values| values.is_empty()) { return Err(AppError::Message(format!("Choice parameter {} needs choices.", parameter.name))); }
        if let Some(pattern) = &parameter.pattern { Regex::new(pattern).map_err(|error| AppError::Message(format!("Invalid pattern for {}: {error}", parameter.name)))?; }
    }
    let token_re = Regex::new(r"\{\{([A-Za-z][A-Za-z0-9_]*)\}\}").unwrap();
    for step in &runbook.steps {
        if step.program.trim().is_empty() || step.program.contains("{{") { return Err(AppError::Message(format!("{} has an invalid program. Programs cannot contain parameters.", source.display()))); }
        for value in step.args.iter().chain(step.env.values()).chain(step.cwd.iter()) {
            for captures in token_re.captures_iter(value) {
                if !names.contains(&captures[1]) { return Err(AppError::Message(format!("{} references unknown parameter {}.", source.display(), &captures[1]))); }
            }
            let without = token_re.replace_all(value, "");
            if without.contains("{{") || without.contains("}}") { return Err(AppError::Message(format!("{} contains malformed parameter syntax.", source.display()))); }
        }
    }
    for pattern in &runbook.redact_patterns { Regex::new(pattern).map_err(|error| AppError::Message(format!("Invalid redaction pattern: {error}")))?; }
    Ok(())
}
...
fn load_files(root: &Path, files: &[PathBuf]) -> AppResult<Vec<LoadedRunbook>> {
    ...
    for source in files {
        ...
        let mut hasher = Sha256::new(); hasher.update(root.to_string_lossy().as_bytes()); hasher.update([0]); hasher.update(runbook.id.as_bytes());
        let key = hex::encode(hasher.finalize())[..24].to_string();
        loaded.push(LoadedRunbook { key, source: source.clone(), runbook });
    }
    Ok(loaded)
}

fn summary(item: &LoadedRunbook) -> RunbookSummary {
    RunbookSummary { id: item.key.clone(), name: item.runbook.name.clone(), description: item.runbook.description.clone(), tags: item.runbook.tags.clone(), risk: item.runbook.risk.clone(), rollback: item.runbook.rollback.clone(), parameters: item.runbook.parameters.clone(), step_count: item.runbook.steps.len(), source: item.source.display().to_string(), trusted: true }
}
...
fn inspect_path(path: &str) -> AppResult<(DirectoryInspection, Vec<LoadedRunbook>)> {
    ...
    let files = yaml_files(&canonical)?; let digest = directory_digest(&canonical, &files)?; let loaded = load_files(&canonical, &files)?;
    let inspection = DirectoryInspection { path: canonical.display().to_string(), digest, files: files.iter().map(|p| p.display().to_string()).collect(), runbooks: loaded.iter().map(summary).collect(), warnings };
    Ok((inspection, loaded))
}

fn verified_runbooks() -> AppResult<(Vec<LoadedRunbook>, Vec<TrustedDirectory>, Vec<String>)> {
    let mut loaded = Vec::new(); let mut directories = Vec::new(); let mut errors = Vec::new();
    for record in trust_records()? {
        let mut valid = false; let mut error = None;
        ...
        if let Some(message) = &error { errors.push(format!("{}: {}", record.path, message)); }
        directories.push(TrustedDirectory { path: record.path, digest: record.digest, signed_at: record.signed_at, valid, error });
    }
...
#[tauri::command]
fn get_state() -> AppResult<AppState> { current_state() }

#[tauri::command]
fn inspect_directory(path: String) -> AppResult<DirectoryInspection> { Ok(inspect_path(&path)?.0) }

#[tauri::command]
fn trust_directory(path: String, digest: String, acknowledged: bool) -> AppResult<AppState> {
    if !acknowledged { return Err(AppError::Message("Confirm that you own or reviewed the folder before signing it.".into())); }
    let (inspection, _) = inspect_path(&path)?;
    if inspection.digest != digest { return Err(AppError::Message("The folder changed during review. Inspect it again.".into())); }
    let mut records = trust_records()?;
    records.retain(|record| record.path != inspection.path);
    records.push(TrustRecord { path: inspection.path.clone(), digest: digest.clone(), signed_at: Utc::now(), signature: sign(&inspection.path, &digest)? });
    save_trust_records(&records)?; current_state()
}
...
fn value_string(parameter: &Parameter, values: &HashMap<String, Value>) -> AppResult<String> {
    ...
    if raw.as_ref().is_none_or(Value::is_null) || raw.as_ref().and_then(Value::as_str) == Some("") {
        if parameter.required { return Err(AppError::Message(format!("{} is required.", parameter.label))); }
        return Ok(String::new());
    }
    let string = match (&parameter.kind, raw.unwrap()) {
        (ParameterType::Boolean, Value::Bool(value)) => value.to_string(),
        (ParameterType::Boolean, Value::String(value)) if matches!(value.as_str(), "true" | "false") => value,
        (ParameterType::Integer, Value::Number(value)) if value.is_i64() || value.is_u64() => value.to_string(),
        (_, Value::String(value)) => value,
        (_, value) => value.to_string(),
    };
    if string.contains('\0') { return Err(AppError::Message(format!("{} contains a null character.", parameter.label))); }
    if parameter.kind == ParameterType::Integer && !Regex::new(r"^-?\d+$").unwrap().is_match(&string) { return Err(AppError::Message(format!("{} must be a whole number.", parameter.label))); }
    if parameter.kind == ParameterType::Choice && !parameter.choices.as_ref().is_some_and(|choices| choices.contains(&string)) { return Err(AppError::Message(format!("Choose an allowed value for {}.", parameter.label))); }
    if let Some(pattern) = &parameter.pattern { if !Regex::new(pattern).unwrap().is_match(&string) { return Err(AppError::Message(format!("{} does not match its required format.", parameter.label))); } }
    Ok(string)
}

fn resolve_values(runbook: &RunbookFile, values: &HashMap<String, Value>) -> AppResult<HashMap<String, String>> {
    runbook.parameters.iter().map(|parameter| Ok((parameter.name.clone(), value_string(parameter, values)?))).collect()
}
...
fn substitute(template: &str, values: &HashMap<String, String>) -> AppResult<String> {
    ...
    for captures in re.captures_iter(template) {
        let matched = captures.get(0).unwrap(); result.push_str(&template[previous..matched.start()]);
        result.push_str(values.get(&captures[1]).ok_or_else(|| AppError::Message(format!("Missing parameter {}.", &captures[1])))?); previous = matched.end();
    }
    result.push_str(&template[previous..]); Ok(result)
}

fn locate(key: &str) -> AppResult<LoadedRunbook> {
    let (books, _, _) = verified_runbooks()?;
    books.into_iter().find(|book| book.key == key).ok_or_else(|| AppError::Message("That runbook is no longer trusted. Refresh its folder and try again.".into()))
}

fn prepare_loaded(item: &LoadedRunbook, values: &HashMap<String, Value>) -> AppResult<(PreparedRun, HashMap<String, String>)> {
...
fn mask_plan_secrets(plan: &mut PreparedRun, runbook: &RunbookFile, resolved: &HashMap<String, String>) {
    for parameter in &runbook.parameters {
        if parameter.kind != ParameterType::Secret { continue; }
...
fn redact(mut output: String, runbook: &RunbookFile, resolved: &HashMap<String, String>) -> String {
    for parameter in &runbook.parameters { if parameter.kind == ParameterType::Secret { if let Some(secret) = resolved.get(&parameter.name) { if !secret.is_empty() { output = output.replace(secret, "[REDACTED]"); } } } }
    for pattern in &runbook.redact_patterns { if let Ok(regex) = Regex::new(pattern) { output = regex.replace_all(&output, "[REDACTED]").into_owned(); } }
    if output.len() > MAX_OUTPUT_BYTES { output.truncate(MAX_OUTPUT_BYTES); output.push_str("\n[output truncated at 64 KB]"); }
    output
}
...
fn execute_run(runbook_id: String, parameters: HashMap<String, Value>, confirmation: String) -> AppResult<RunResult> {
    ...
    for (index, _) in item.runbook.steps.iter().enumerate() {
        ...
        if let Some(cwd) = &prepared.cwd {
            let canonical = fs::canonicalize(cwd).map_err(|_| AppError::Message(format!("Working directory does not exist: {cwd}")))?;
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            TrayIconBuilder::new().icon(app.default_window_icon().expect("app icon").clone()).menu(&menu)
                .on_menu_event(|app, event| match event.id.as_ref() { "quit" => app.exit(0), "open" => show_window(app), _ => {} })
                .on_tray_icon_event(|tray, event| { if let TrayIconEvent::Click { button: MouseButton::Left, button_state: MouseButtonState::Up, .. } = event { show_window(tray.app_handle()); } })
                .build(app)?;
            Ok(())
        })
...
fn show_window(app: &AppHandle) { if let Some(window) = app.get_webview_window("main") { let _ = window.show(); let _ = window.set_focus(); } }

#[cfg(test)]
mod tests {
    use super::*;

    fn sample() -> &'static str { r#"version: 1
id: clear-cache
name: Clear cache
...
"# }
    ...
    fn parses_and_prepares_direct_argv() {
        let directory = tempfile::tempdir().unwrap(); let file = directory.path().join("clear.yaml"); fs::write(&file, sample()).unwrap();
        let files = yaml_files(directory.path()).unwrap(); let books = load_files(directory.path(), &files).unwrap();
        let mut values = HashMap::new(); values.insert("cache".into(), Value::String("pages; rm -rf /".into()));
        ...
        assert_eq!(plan.steps[0].program, "printf"); assert_eq!(plan.steps[0].args[1], "pages");
    }

    #[test]
    fn digest_changes_when_yaml_changes() {
        let directory = tempfile::tempdir().unwrap(); let file = directory.path().join("a.yaml"); fs::write(&file, sample()).unwrap(); let files = yaml_files(directory.path()).unwrap();
        let first = directory_digest(directory.path(), &files).unwrap(); fs::write(&file, sample().replace("pages]", "pages, api]")).unwrap();
