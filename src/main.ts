...
function renderSettings(): void {
  ...
  workspace.innerHTML = `<section class="page settings"><p class="eyebrow">Device field notes</p><h1>Settings</h1>${folders}<section><h2>License</h2><p>${license.unlocked ? "License active. Unlimited runbooks and the full 100-entry history are available." : "The free field kit includes up to 3 runbooks and 10 history entries. A $29 one-time license adds unlimited local runbooks and extended history."}</p>${!license.unlocked ? `<p><a class="button primary" href="https://api.sociobot.in/api/v1/products/hotkey-runbook/checkout" target="_blank" rel="noreferrer">Buy the one-time license</a></p><form id="license-form"><label class="field"><span class="field-label"><span>Have a license? Paste it</span></span><input id="license-token" type="password" required autocomplete="off"></label><div class="inline-actions"><button class="button">Verify license</button></div><p class="muted">Sociobot and Dodo handle checkout and refunds.</p><p id="license-status" aria-live="polite"></p></form>` : `<button class="text-button" id="remove-license">Remove license from this device</button>`}</section><section><h2>Installed build</h2><p class="muted">Version ${esc(buildIdentity.version)} · source ${esc(buildIdentity.commit)}</p><code tabindex="0" aria-label="Installed build identity">hotkey-runbook --build-identity</code></section><section><h2>Privacy and safety</h2><p>No account, telemetry, remote orchestration, or secret vault. Commands run as direct local processes only after review. History is stored in the app data directory.</p></section></section>`;
